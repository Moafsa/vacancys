...
describe('EmailService', () => {
  ...
  const anotherMockUser = new User({
    id: '456',
    name: 'Jane Doe',
  ...
  beforeEach(() => {
    ...
    emailService = new EmailService(mockUserRepository);
  });
  ...
  describe('Constructor', () => {
    it('should create instance with default SMTP port when not provided', () => {
      ...
      const service = new EmailService(mockUserRepository);
  ...
  describe('getUserRepository', () => {
    ...
    it('should create a new repository if none was provided in constructor', () => {
      const serviceWithoutRepo = new EmailService();
  ...
  describe('verifyEmail', () => {
    it('should verify user email when token is valid', async () => {
      ...
      (emailService as any).userRepository = mockUserRepository;

      const token = jwt.sign({ email: anotherMockUser.email }, JWT_SECRET);
    ...
    it('should throw error for non-existent user', async () => {
      const token = jwt.sign({ email: 'nonexistent@example.com' }, JWT_SECRET);
    ...
    it('should throw error when email is already verified', async () => {
      ...
      const token = jwt.sign({ email: mockUser.email }, JWT_SECRET);
