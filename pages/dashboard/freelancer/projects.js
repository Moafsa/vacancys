      ...
      <div className="max-w-5xl mx-auto py-8 px-4">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-green-800 flex items-center">
            <BriefcaseIcon className="h-7 w-7 mr-2 text-green-500" /> Available Projects
          </h2>
        </div>
          ...
          {projects.map(project => (
            <div key={project._id} className="bg-white shadow-lg rounded-lg p-6 hover:shadow-xl transition-all border-l-4 border-green-500">
              ...
              <div className="flex items-center justify-between mt-4">
                <Link href={`/dashboard/freelancer/projects/${project._id}/propose`} className="inline-flex items-center px-3 py-1.5 border border-green-600 text-green-600 bg-white rounded hover:bg-green-50 text-sm font-medium transition-all">
                  Send Proposal
                </Link>
              </div>
            </div>
